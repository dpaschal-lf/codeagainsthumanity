...
class App extends Component {
  constructor(props){
    ...
    this.readyToggle = this.readyToggle.bind(this);
  ...
  getPlayerReadyList(nameList, readyList){
  	return nameList.map( (name, index) => <PlayerName name={name} index={index} key={index} ready={readyList[index]}/>)
  }
  mode_judge(){
  	return ( 
  ...
  mode_cardWaiting(){
  	return(
  		<div>
	  		<Waiting message={this.state.message}/>
	  		<div> waiting to play: </div>
	  		{ this.getPlayerReadyList(this.state.players,this.state.cardReadyStates) }
	  	</div>
  	)
  }
  acceptPlayerAnswers(selectedIndex){
  	console.log('judge picked cards');
  	...
  	return (
  		<div>
  			time for you to pick
  			 <Hand cardText={this.state.questions} type="question" currentAnswers={this.state.currentAnswers.map(item=>'__')}/>
  			 { this.state.cardSelections.map( (selections, index)=> 
  			 	<Hand key={index} cardText={selections} type="answer" index={index} submit={this.acceptPlayerAnswers.bind(this, index)} />
  			 	)}
  	...
  	return (
  		<div>
 			...
 			<div className="scores">
 				{this.state.scores.map( (data, index)=> <div key={index}>{`${data.name}: ${data.score}`}</div>)}
 			</div>
  			<button onClick={this.handleReplay}>Play Again</button><button onClick={this.handleQuit}>Quit</button>
  		</div>
  	)
  }
  handleReplay(){
  ...
  acceptAnswer(answerIndices){
  	console.log('answer is', answerIndices);
  	this.server.send({
  		game: this.state.gameCode, 
  		playerData: { 
  			chosenAnswers: answerIndices
  		}
  	}, 'chooseAnswers')   	
  }
  ...
  receiveMessage(packet){
  	...
  	for(let key in packet.data){
  		if(this.validStateKeys.indexOf(key)===-1){
  ...
  render() {
  	return (
  		<div className="App">
  			<div className="yourName">{this.state.name}</div>
